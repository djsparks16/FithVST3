...
void FiveParksVST3AudioProcessor::prepareToPlay(double sampleRate, int samplesPerBlock)
{
    ...
    reverbLineL.assign((size_t) (sampleRate * 0.35), 0.0f);
    reverbLineR.assign((size_t) (sampleRate * 0.43), 0.0f);
    chorusWrite = delayWrite = reverbWrite = 0;
...
void FiveParksVST3AudioProcessor::processBlock(juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midi)
{
    ...
    for (int sample = 0; sample < numSamples; ++sample)
    {
        ...
        const int revReadL = (reverbWrite - (int) (0.11 * currentSampleRate) + (int) reverbLineL.size()) % (int) reverbLineL.size();
        const int revReadR = (reverbWrite - (int) (0.17 * currentSampleRate) + (int) reverbLineR.size()) % (int) reverbLineR.size();
        const float rvL = reverbLineL[(size_t) revReadL];
        const float rvR = reverbLineR[(size_t) revReadR];
        reverbLineL[(size_t) reverbWrite] = l + rvR * 0.55f;
        reverbLineR[(size_t) reverbWrite] = r + rvL * 0.55f;
        reverbWrite = (reverbWrite + 1) % (int) reverbLineL.size();
        l += rvL * p.reverbMix;
        r += rvR * p.reverbMix;
